...
def eval_mmdet(config_py='configs/pascal_voc/faster_rcnn_r50_fpn_1x_voc0712.py', pretrained_model='work_dirs/faster_rcnn_r50_fpn_1x_voc0712/epoch_100.pth', 
    ...
    if mmdet.__version__.startswith('0.6.0'):
        ...
        cfg = mmcv.Config.fromfile(config_py)
        cfg.model.pretrained = None

        # construct the model and load checkpoint
        model = build_detector(cfg.model, test_cfg=cfg.test_cfg)
        _ = load_checkpoint(model, pretrained_model)
        ...
        for i, result in enumerate(inference_detector(model, imgs, cfg, device='cuda:0')):
            ...
            print(i, imgs[i])
            show_result(imgs[i], result, dataset='voc', score_thr=0.5, out_file=os.path.join('./eval_mmdet_imgs', imgs[i].split('/')[-1]))
            # print('write to {0}'.format(os.path.join('./eval_mmdet_imgs', imgs[i].split('/')[-1])))
